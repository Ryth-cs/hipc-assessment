 ...
 * 
 */
void allocate_arrays(int recv_count, int col_length, int rank) {
	if (rank == 0) {
...
void problem_set_up(int rank, int recv_count, int displacement, int col_length) {
    for (int i = displacement; i < displacement+recv_count; i++ ) {
        for (int j = 0; j < Ex_size_y; j++) {
            ...
            ex_local[i-displacement][j] = mag * tx;
		}
	}
    for (int i = displacement; i < displacement+recv_count+1; i++ ) {
        for (int j = 0; j < Ey_size_y; j++) {
            ...
            ey_local[i-displacement][j] = mag * ty;
		}
	}
}
